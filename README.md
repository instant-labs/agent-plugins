...
| --- | --- | --- |
| [Instant](plugins/instant) | Domain availability, name ideas, bulk checks, and USPTO trademark search | `https://mcp.instant.ai/mcp`, no auth |
...
The plugin, primary skill, and MCP server are renamed to `instant`.
The marketplace remains `instant-marketplace`. Existing `instantdomainsearch`
installs do not automatically switch to the new plugin.
...
In other clients, remove the old **Instant Domain Search** plugin, refresh the
marketplace, and install **Instant** using the instructions above. Restart the
client or start a new session to load the renamed skills and tools. Claude Code
commands now use `/instant:domain` and `/instant:domain-variations`.

For skills-only installs, remove the old `instantdomainsearch` skill and reinstall
the skills. If you configured the MCP server manually, replace its entry with
`instant` at `https://mcp.instant.ai/mcp`.
